...
#: стоп-слова, общие для любой ниши: закрытые точки, вендинг, служебные объекты
_GENERIC_BLOCKLIST = (
    "закрыто", "закрыт", "не работает", "ликвидирован",
    "автомат", "vending", "вендинг", "торговый автомат",
    "киоск", "ларек", "ларёк", "торговый павильон",
    "общежитие", "воинская часть",
)

#: стоп-слова только для общепита — в других нишах они законные названия
_FOOD_BLOCKLIST = (
    ...
    "павильон быстрого питания", "кофейный автомат",
    ...
    "аптека", "автомойка", "шиномонтаж", "хостел",
)
...
#: родовые названия без бренда — карточка, которой нечего продавать
_GENERIC_NAMES = {
    ...
    "магазин", "салон", "студия", "клиника", "аптека", "офис", "мастерская",
    "парикмахерская", "автосервис", "автомойка", "сто", "барбершоп",
    "стоматология", "фитнес", "отель", "гостиница", "хостел", "типография",
    "цветы", "мебель", "ремонт", "услуги", "продукты",
}


def name_is_generic(name: str) -> bool:
    """Название вида «Кафе», «Салон», «Клиника» — карточка без бренда."""
    stripped = re.sub(r"[^0-9a-zа-яё ]+", "", (name or "").lower()).strip()
    return stripped in _GENERIC_NAMES
...
def exclusion_reason(
    *,
    ...
    """Причина исключения записи или None, если бизнес подходит.

    ``food=False`` отключает стоп-слова общепита: для ниши «аптеки» слово
    «аптека» в названии — норма, а не повод выбросить запись.
    """
    tags = tags or {}
    ...
    blocklist = _GENERIC_BLOCKLIST + (_FOOD_BLOCKLIST if food else ())
