  ...
  return (
    <section className="rounded-2xl border border-line bg-white p-5">
      ...
      <div className="mt-4 flex gap-3">
        ...
        <div className="flex h-44 w-11 shrink-0 flex-col justify-between text-right text-[11px] tabular-nums text-ink-muted">
          {ticks.map((tick) => (
        ...
        <div className="min-w-0 flex-1">
                ...
                return (
                  ...
                  >
                    ...
                    {index === peakIndex && hovered === null ? (
                      <span
                        className="pointer-events-none absolute left-1/2 z-[1] w-max -translate-x-1/2 text-[11px] font-bold tabular-nums text-ink-soft"
                    ...
                    {active ? (
                      <div className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-2 w-max -translate-x-1/2 rounded-xl border border-line bg-white px-3 py-2 text-left shadow-[0_18px_40px_-20px_rgba(38,33,25,0.35)]">
                        <p className="text-sm font-bold tabular-nums">{money(point.revenue)}</p>
          ...
          <div className="mt-2 flex h-4 gap-[2px]">
            {points.map((point, index) => (
              ...
              >
                {index % labelEvery === 0 ? (
                  <span className="absolute top-0 left-1/2 -translate-x-1/2 text-[11px] whitespace-nowrap text-ink-muted">
      ...
      <details className="mt-3 text-sm">
        <summary className="cursor-pointer text-xs text-ink-muted">Показать таблицей</summary>
