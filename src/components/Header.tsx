...
export function Header() {
  ...
  const link = solid
    ...
    : "text-cream/75 transition-colors hover:text-cream";
  const activeLink = solid ? "font-semibold text-basil" : "font-semibold text-cream";
  ...
  return (
    <header
      ...
      className={`sticky top-0 z-40 border-b transition-colors duration-500 ${
        solid
          ? "border-cream-dark/80 bg-cream/85 backdrop-blur"
    ...
    >
      <div className="mx-auto flex h-16 max-w-6xl items-center gap-4 px-4 sm:px-6">
        ...
        <Link
          href="/cart"
          className={`ml-auto inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-semibold transition-colors duration-500 sm:ml-0 ${
            solid
              ? "bg-basil text-cream hover:bg-basil-dark"
              : "bg-cream text-basil-dark hover:bg-white"
          }`}
          ...
          {hydrated && count > 0 && (
            ...
            <motion.span
              ...
              className={`inline-flex min-w-6 justify-center rounded-full px-1.5 text-xs font-bold ${
                solid ? "bg-cream text-basil" : "bg-basil text-cream"
              }`}
            >
              {count}
            </motion.span>
      ...
      <nav
        className={`flex gap-5 border-t px-4 py-2.5 text-sm transition-colors duration-500 sm:hidden ${
          solid ? "border-cream-dark/70" : "border-cream/15"
        }`}
      >
        {NAV.map((navLink) => (
          <Link
        ...
        <a href={`tel:${RESTAURANT.phoneHref}`} className={`ml-auto ${link}`}>
          Позвонить
        </a>
      </nav>
    </header>
