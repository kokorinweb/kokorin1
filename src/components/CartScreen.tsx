...
/**
 * Экран корзины. Стоп-лист приходит пропом с сервера: корзина живёт в localStorage
 * браузера, поэтому блюдо могло кончиться уже после того, как гость его добавил.
 * Предупредить об этом до оформления честнее, чем отказать после нажатия кнопки.
 */
export function CartScreen({ unavailable }: { unavailable: Record<string, string> }) {
  ...
  const stopped = entries.filter((entry) => unavailable[entry.item.id] !== undefined);
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-28 pt-14 sm:px-6">
      ...
      {stopped.length > 0 && (
        <div className="mt-6 rounded-2xl border border-terracotta/30 bg-terracotta/10 p-4 text-sm">
          <p className="font-semibold text-terracotta">
            {stopped.length === 1
              ? `«${stopped[0]!.item.name}» сегодня закончилось`
              : `Сегодня закончились: ${stopped.map((entry) => `«${entry.item.name}»`).join(", ")}`}
          </p>
          <p className="mt-1 text-ink-soft">
            Уберите из корзины, чтобы оформить заказ, — или позвоните нам, подберём замену:{" "}
            <a href={`tel:${RESTAURANT.phoneHref}`} className="underline">
              {RESTAURANT.phone}
            </a>
            .
          </p>
      ...
      <div className="mt-10 grid gap-10 lg:grid-cols-[1.4fr_1fr]">
          ...
          {entries.map((entry) => (
            <li
              ...
              className={`flex flex-wrap items-center gap-4 rounded-2xl border p-4 ${
                unavailable[entry.item.id] !== undefined
                  ? "border-terracotta/30 bg-terracotta/5"
                  : "border-cream-dark bg-white/70"
              }`}
            >
        ...
        >
          ...
          <button
            type="submit"
            disabled={sending || belowMinimum || stopped.length > 0}
